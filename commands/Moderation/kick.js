...
module.exports = {
  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    ...
    if (!member) return message.reply({ embeds: [sp] });

    if (member == null) return message.channel.send("null");
    ...
    const position = new MessageEmbed()
      .setColor("RED")
      .setDescription(
        "akhi you cannot kick somone who is higher rank then you"
      );
    ...
    if (!member.bannable) return message.reply({ embeds: [kickable] });
