...
module.exports = {
  ...
  run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
    ...
    collector.on("collect", async (i) => {
      ...
      if (i.values[0] === "announcement" || i.value[1] === "announcement") {
        ...
        i.member.roles.remove("889163405842350140");
          ...
          roleadd.push(announcement)
          i.member.roles.add("889163405842350140");
        
        }
      
      }
      if (i.values[0] === "partnership" || i.value[1] === "partnership") {
      ...
      if (
        ...
        i.value[1] === "ping for dead server"
      ) {
        if (i.member.roles.cache.has(dead.id)) {
        ...
        .setColor("BLUE")
        .setDescription(`Added the role(s): ${roleadd.join(",")}/nRemoved the Role(s): ${roleremoved.join(",")} `)
